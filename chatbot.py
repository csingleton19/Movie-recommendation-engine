...
def handle_user_message(user_message):
    ...
    # If the user is asking for recommendations, fetch them
    if "recommend" in user_message:
        recommendations = []
        if user_preferences.get('movie'):  # if a specific movie title has been specified
            recommendations = find_similar_movies(user_preferences)
        elif any(item in user_message for item in ['genre', 'director', 'actor']):  # if any specified feature is in the user message
            recommendations = find_similar_movies_by_preferences(movies_df, user_preferences)
        
        # Convert tuples in recommendations to strings and add to the assistant message
        if recommendations:
            recommendation_strings = [', '.join(map(str, r)) for r in recommendations]
            assistant_message = "Here are some recommendations: " + "; ".join(recommendation_strings)
